...
void DetectionOutputLayer<Dtype>::Forward_gpu(
  ...
  Dtype* pose_permute_data = pose_permute_.mutable_gpu_data();
  PermuteDataGPU<Dtype>(bottom[6]->count(), bottom[6]->gpu_data(),
        aspect_classes_, num_priors_, 1, pose_permute_data);

  const Dtype* conf_cpu_data = conf_permute_.cpu_data();
  const Dtype* pose_cpu_data = pose_permute_.cpu_data();
  //
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int c = 0; c < num_classes_; ++c) {
      ...
      // store the indices for every classes
    }
  ...
  top_shape.push_back(8);//[1,1,num_kept,8]
  Dtype* top_data;
  if (num_kept == 0) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        //change here
        const int pose_label = 0;
        const float max_pose_conf = 0;
        for (int cur_pose = 0; cur_pose < 4; cur_pose++) {
          max_pose_conf = max_pose_conf > cur_pose_data[idx * 4 + cur_pose] ? max_pose_conf : cur_pose_data[idx * 4 + cur_pose];
        ...
        if (need_save_) {
          ...
          NormalizedBBox bbox;
          bbox.set_xmin(top_data[count * 8 + 3]);
          bbox.set_ymin(top_data[count * 8 + 4]);
          bbox.set_xmax(top_data[count * 8 + 5]);
          bbox.set_ymax(top_data[count * 8 + 6]);
